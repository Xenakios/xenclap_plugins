...
class KlangasGUI
{
  public:
    ...
    KlangasGUI(std::vector<ParamDesc> paramDescs, CommFIFO &outfifo, CommFIFO &infifo,
...
struct xen_klangas : public clap::helpers::Plugin<clap::helpers::MisbehaviourHandler::Terminate,
                                                  clap::helpers::CheckingLevel::Maximal>
{
    ...
    AdditiveSynth m_synth;

    xen_klangas(const clap_host *host, const clap_plugin_descriptor *desc)
                  ...
                  uint32_t maxFrameCount) noexcept override
    {
        ...
        m_synth.m_shared_data.setVolumeMorphPreset(1);
        m_synth.m_shared_data.setPanMorphPreset(0);
        m_synth.setEDOParameters(1200.0, 12);
        return true;
    }
  ...
  protected:
    ...
    {
        if (value && paramId >= 0 && paramId < paramValues.size())
        {
    ...
    void handleNextEvent(const clap_event_header_t *nextEvent, bool is_from_ui)
    {
        ...
        case CLAP_EVENT_NOTE_CHOKE:
        {
            ...
            m_synth.handleNoteOff(nevt->port_index, nevt->channel, nevt->key, nevt->note_id);
            break;
        }
        case CLAP_EVENT_NOTE_ON:
        {
            auto nevt = reinterpret_cast<const clap_event_note *>(nextEvent);
            m_synth.handleNoteOn(nevt->port_index, nevt->channel, nevt->key, nevt->note_id,
                                 nevt->velocity);
            break;
        }
        case CLAP_EVENT_NOTE_EXPRESSION:
        {
            ...
            // m_synth.applyNoteExpression(nexp->port_index, nexp->channel, nexp->key,
            // nexp->note_id,
        ...
        case CLAP_EVENT_PARAM_VALUE:
        {
            ...
            if (pevt->param_id >= 0 && pevt->param_id < paramValues.size())
            {
                paramValues[pevt->param_id] = pevt->value;
                m_synth.handleParameterValue(pevt->port_index, pevt->channel, pevt->key,
                                             pevt->note_id, pevt->param_id, pevt->value);
                /*
    ...
    clap_process_status process(const clap_process *process) noexcept override
    {
        ...
        uint32_t chunkSize = SRProvider::BLOCK_SIZE;
        uint32_t pos = 0;
                                             ...
                                             const char *plugin_id)
{
    ...
    auto wr = new xen_klangas(host, &desc);
